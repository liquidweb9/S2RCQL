...
def traverse_environment(env: gym.Env
    ...
    for idx, i in enumerate(task_set):
        ...
        model.reset()
        observation: str = env.reset(target_location=target_location, agent_location=agent_location)
...
def train(
    ...
    parser.add_argument("--trainsetb", default=10, type=int)
...
if __name__ == "__main__":
    env = gym.make("MazeWorld-v0", size=5, obstacles_num=3, obstacles_location=np.array([[2, 0], [1, 1], [1, 2]]))
    train(agent_location=np.array([4, 0]), target_location=np.array([1, 0]), env=env, )
